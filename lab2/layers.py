...
class FC(Layer):
  ...
  def backward_params(self, grads):
    ...
    """
    grad_weights = np.matmul(grads.T, self.last_inputs)
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    grads[self.last_inputs <= 0.0] = 0.0
    return grads
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    exp_x = np.exp(x)
    sum_of_exp = np.sum(exp_x, axis=1)
    log_of_sum = np.log(sum_of_exp)
    
    sum_of_mul = np.sum(x * y, axis=1)
    
    return np.mean(log_of_sum - sum_of_mul)
  ...
  def backward_inputs(self, x, y):
    ...
    softmax_x = (exp_x.T / sum_of_exp).T
    
    return (softmax_x - y) / N
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    norm_of_weights = np.linalg.norm(self.weights)
    return 0.5 * self.weight_decay * np.power(norm_of_weights, 2)
